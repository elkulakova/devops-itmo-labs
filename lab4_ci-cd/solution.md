...
- отсутствие явной стуктурированной зависимости между job-ами: этапы выполняются независимо и параллельно, даже если предыдущие завершились с ошибкой. Это нарушает принцип fail-fast, увеличивает время обнаружения ошибок и приводит к неэффективному расходованию ресурсов, так как последующие этапы (например, сборка или деплой) могут выполняться при некорректном состоянии проекта;
...
Не отчаиваемся, удаляем предателя и ищем более патриотичный аналог.
...
ssh -p 443 -R 80:localhost:8200 a.pinggy.io
 ...
 Your tunnel will expire in 60 minutes. Upgrade to Pinggy Pro to get unrestricted tunnels. https://dashboard.pinggy.io  
   ...
   http://kwwtj-ip-address.run.pinggy-free.link                                                                      
   https://kwwtj-ip-address.run.pinggy-free.link  
```
...
Теперь в репозитории `GitHub` в настройках проекта добавим секрет `VAULT_TOKEN` со значением нашего токена, а также `VAULT_URL` со значением `https://kwwtj-ip-address.run.pinggy-free.link` (адрес нашего хранилища). Теперь мы можем использовать эти секреты в нашем CI/CD pipeline для доступа к `Vault` и получения наших секретов во время выполнения.
...
Запушим изменения в репозиторий и насладимся результатом:
...
Как видим, в логах ничего не засветилось, значит, все правильно сделала!

![img_9.png](img_9.png)

### Почему способ с Vault «красивый»:
- Централизация управления секретами: Все секреты хранятся в одном месте (`Vault`), что упрощает управление и аудит. Можно легко обновлять секреты в одном месте, и эти изменения будут автоматически применяться во всех местах, где они используются. Также универсальный доступ к хранилищу => упрщение работы с секретами.
- Маскировка «из коробки»: `hashicorp/vault-action` сам помечает все скачанные данные как секреты. Тебе не нужно вручную прописывать маскировку для каждой переменной.
- Отсутствие следов в настройках `GitHub`: В переменных репозитория лежит только один «ключ от сейфа» (`VAULT_TOKEN`). Самих рабочих токенов (от БД, деплоя, API) в GitHub нет. Если злоумышленник взломает аккаунт `GitHub`, он не увидит твои токены — они спрятаны за туннелем в локальном хранилище по сути на локальной машине.
- Динамическая область видимости: Секрет из `Vault` попадает только в память конкретной джобы. Он не сохраняется на диске раннера и исчезает сразу после завершения работы.

### Почему хранение в CI/CD переменных репозитория — это «Bad Practice»?
- Static Lifetime (Статичность): Секреты в `GitHub Secrets` — это «вечные» строки. Они лежат там, пока их не удалят. В `Vault` можно настроить `TTL` (Time To Live): секрет может быть валиден только 5 минут. Доступ к `Vault` осуществляется через временный публичный `URL`, предоставляемый сервисом туннелирования (`pinggy`). Несмотря на то, что `Vault` развернут локально, туннель делает его доступным извне по `HTTPS`. Таким образом, безопасность решения определяется защитой `VAULT_TOKEN` и временем жизни туннеля (по дефолту оно всего 60 минут). Использование `Vault` позволяет избежать хранения самих секретов в `GitHub`, однако не исключает полностью риски компрометации при утечке токена доступа.
- No Central Governance (Отсутствие централизации): При большом кличестве проектов или микросервисов обновлять один и тот же пароль в настройках каждого репозитория вручную себе дороже — это еще и риск человеческой ошибки. `Vault` позволяет менять секрет в одной папке для всех, а в репозиториях/микросервисах просто будет ссылочка на серкет в хранилище.
- Риск «Insider Threat»: Администратор репозитория в `GitHub` может легко подсмотреть значение секрета, просто создав свой пайплайн и выведя его в лог (если не настроена сложная защита). `Vault` позволяет настроить гораздо более строгие политики доступа.