...
4) Проверим, как наш сервис поживает на просторах необъятного. Так как `minicube` автоматически в качестве драйвера назначил `docker`, надо проложить путь к сервису:
   ```
   liza@liza-ai-pc:~/work/itmo/devops/devops-labs$ kubectl port-forward service/local-app-service 8080:80
   Forwarding from 127.0.0.1:8080 -> 80
   Forwarding from [::1]:8080 -> 80
   Handling connection for 8080
   ```
   Видим базовую страничку `nginx`, который и был использован в качестве образа, значит, успех!
